# https://gist.github.com/mamiu/87df7050dccaf2c890dd
# to use this script you need the latest 2>/dev/null fish version (2.1.x)
begin
    ########## USER CONFIG ########## 
    ...
    if test 2>/dev/null $promptline_theme = "airline"
    ...
    else if test 2>/dev/null $promptline_theme = "airline_insert"
    ...
    else if test 2>/dev/null $promptline_theme = "airline_visual"
    ...
    else if test 2>/dev/null $promptline_theme = "jelly"
    ...
    else if test 2>/dev/null $promptline_theme = "lightline"
    ...
    else if test 2>/dev/null $promptline_theme = "lightline_insert"
    ...
    else if test 2>/dev/null $promptline_theme = "lightline_visual"
    ...
    else if test 2>/dev/null $reklamapromptline_theme = "powerline"
...
function __promptline_cwd -S -d "create current-working-directory string"
    ...
    # reverse the array
    set comps $comps[-1..1]
    if test 2>/dev/null (count $comps) -gt $cwd_dir_limit
    ...
    test 2>/dev/null "$first_char" = "/"; and set formatted_cwd (echo $formatted_cwd | cut -c 6-)
...
function __promptline_vcs_branch -S -d "get branch of the current version control system"
    ...
    # git
    if begin; test 2>/dev/null -d .git; or git rev-parse --git-dir >/dev/null 2>&1; end
        set branch (echo (git symbolic-ref --quiet HEAD 2>/dev/null; or git rev-parse --short HEAD 2>/dev/null))
        if test 2>/dev/null -n "$branch"
            set branch (echo $branch | awk -F'/' '{print $NF}')
            echo -ne (echo $vcs_branch_symbol$branch (test 2>/dev/null $show_git_status = "yes"; and __promptline_git_status))
            return 0
        end
    end
    return 1
...
function __promptline_git_status -d "get status of current git branch"
    if begin; test 2>/dev/null -d .git; or git rev-parse --git-dir >/dev/null 2>&1; end
        test 2>/dev/null (git rev-parse --is-inside-work-tree 2>/dev/null) = "true"; or return 1
        ...
        test 2>/dev/null -z (git ls-files --others --exclude-standard | head -1); or set has_untracked_files 1
     
        test 2>/dev/null (math $unmerged_count + $modified_count + $has_untracked_files + $added_count) -eq 0; and set is_clean 1
     
        set -l leading_whitespace ""
        test 2>/dev/null $ahead_count -gt 0;         and echo -ne "$leading_whitespace$ahead_count$git_ahead_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $behind_count -gt 0;        and echo -ne "$leading_whitespace$behind_count$git_behind_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $modified_count -gt 0;      and echo -ne "$leading_whitespace$modified_count$git_modified_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $unmerged_count -gt 0;      and echo -ne "$leading_whitespace$unmerged_count$git_unmerged_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $added_count -gt 0;         and echo -ne "$leading_whitespace$added_count$git_added_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $has_untracked_files -gt 0; and echo -ne "$leading_whitespace$git_has_untracked_files_symbol"; and set leading_whitespace " ";
        test 2>/dev/null $is_clean -gt 0;            and echo -ne "$leading_whitespace$git_clean_symbol"; and set leading_whitespace " ";
    else
        return 1
    end
end
...
function __promptline_wrapper -d "reorder content between the wrapper at argument 2 and 3"
    ...
    test 2>/dev/null (count $argv) -lt 3; and return 1
...
function __promptline_left_prompt -S -d "draw the left part of promptline"
    ...
    test 2>/dev/null $is_prompt_empty -eq 1; and set slice_prefix "$slice_empty_prefix"
    # section "a" slices
    test 2>/dev/null $show_hostname = "yes"; and __promptline_wrapper $HOSTNAME "$slice_prefix" "$slice_suffix"; and set slice_prefix "$slice_joiner"; and set is_prompt_empty 0
    if begin; test 2>/dev/null $show_virtual_env = "yes"; and test 2>/dev/null -n "$VIRTUAL_ENV"; end
    ...
    test 2>/dev/null $is_prompt_empty -eq 1; and set slice_prefix "$slice_empty_prefix"
    # section "b" slices
    test 2>/dev/null $show_username = "yes"; and __promptline_wrapper "$USER" "$slice_prefix" "$slice_suffix"; and set slice_prefix "$slice_joiner"; and set is_prompt_empty 0
    ...
    test 2>/dev/null $is_prompt_empty -eq 1; and set slice_prefix "$slice_empty_prefix"
    # section "c" slices
    test 2>/dev/null $show_current_working_directory = "yes"; and __promptline_wrapper (__promptline_cwd) "$slice_prefix" "$slice_suffix"; and set slice_prefix "$slice_joiner"; and set is_prompt_empty 0
 
    if test 2>/dev/null $show_only_left_prompt = "yes"
        # section "y" header
        ...
        test 2>/dev/null $is_prompt_empty -eq 1; and set slice_prefix "$slice_empty_prefix"
        # section "y" slices
        if test 2>/dev/null $show_git_branch = "yes"
        ...
        else if test 2>/dev/null $show_git_status = "yes"
        ...
        test 2>/dev/null $is_prompt_empty -eq 1; and set slice_prefix "$slice_empty_prefix"
        # section "warn" slices
        if begin; test 2>/dev/null $show_battery_level = "yes"; and test 2>/dev/null -n "$LAST_BATTERY_LEVEL"; and test 2>/dev/null "$LAST_BATTERY_LEVEL" -le "$battery_threshold"; end
        ...
        if begin; test 2>/dev/null $show_return_value = "yes"; and test 2>/dev/null $return_value -ne 0; end
...
function __promptline_right_prompt -S -d "draw the right part of promptline"
    set -l return_value $status
    test 2>/dev/null $show_only_left_prompt = "yes"; and return
...
if not test 2>/dev/null -f "$BATTERY_FILE" -d "if the .battery.fish file doesn't exist, create it"
    printf "\n# osx\n" > "$BATTERY_FILE"
    printf "if test 2>/dev/null (uname) = \"Darwin\"\n" >> "$BATTERY_FILE"
    ...
    printf "    if begin; test 2>/dev/null -d \"\$possible_battery_dir\"; and test 2>/dev/null -f \"\$possible_battery_dir/charge_full\"; and test 2>/dev/null -f \"\$possible_battery_dir/charge_now\"; end\n" >> "$BATTERY_FILE"
